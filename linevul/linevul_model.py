...
class Model(RobertaForSequenceClassification):
    ...
    def __init__(self, encoder, config, tokenizer, args):
        ...
        self.encoder:RobertaForSequenceClassification = encoder#编码器
        self.tokenizer = tokenizer#
        self.classifier = RobertaClassificationHead(config)#下游任务：2分类
        self.args = args
    ...
    def forward(self, input_embed=None, labels=None, output_attentions=False, input_ids=None):
        if output_attentions:#是否输出注意力分数（训练时不需要)
            if input_ids is not None:#
                ...
                """
                outputs = self.encoder.roberta(input_ids, attention_mask=input_ids.ne(1), output_attentions=output_attentions)#相当于调用roberta的__call__方法，进行前向传播
            else:#已经进行词嵌入的情况
