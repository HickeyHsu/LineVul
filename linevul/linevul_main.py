...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs): 
        bar = tqdm(train_dataloader,total=len(train_dataloader))# 进度条
        tr_num = 0
        train_loss = 0
        for step, batch in enumerate(bar):
            (inputs_ids, labels) = [x.to(args.device) for x in batch]#读取输入样本id和label到device中
            model.train()#训练
            loss, logits = model(input_ids=inputs_ids, labels=labels)#输出loss
            if args.n_gpu > 1:#多GPU
                loss = loss.mean()
            if args.gradient_accumulation_steps > 1:#用同一个batch多次进行梯度计算并进行累加后再更新网络参数
                loss = loss / args.gradient_accumulation_steps

            loss.backward()#反向更新网络参数
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)#XXX: 正则化？待研究
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:#XXX: 保存这个epoch的结果
                optimizer.step()
                optimizer.zero_grad()
                scheduler.step()  
                global_step += 1
                ...
                if global_step % args.save_steps == 0:#每训练save_steps步之后eval一次
